...
* JS Device Detection
*/
if(!sessionStorage.device) {
	window.device = {};
	window.device.agent = navigator.userAgent.toLowerCase();
	window.device.scrWidth = screen.width;
	window.device.scrHeight = screen.height;
	window.device.otherBrowser = (window.device.agent.indexOf("series60") != -1) || (window.device.agent.indexOf("symbian") != -1) || (window.device.agent.indexOf("windows ce") != -1) || (window.device.agent.indexOf("blackberry") != -1);
	window.device.mobileOS = typeof orientation != 'undefined' ? true : false;
	window.device.touchOS = ('ontouchstart' in document.documentElement) ? true : false;
	window.device.iOS = (navigator.platform.indexOf("iPhone") != -1) ||
		        (navigator.platform.indexOf("iPad") != -1) || (navigator.platform.indexOf("iPod") != -1) ? true : false;
	window.device.android = (window.device.agent.indexOf("android") != -1) || (!window.device.iOS && !window.device.otherBrowser && window.device.touchOS && window.device.mobileOS) ? true : false;
		
sessionStorage.device = JSON.stringify(window.device);
}
else {
	window.device = JSON.parse(sessionStorage.device);
}
if (window.device.mobileOS === true) {
	var bodyClass = 'mobile';
}
else {
...
$(document).ready(function() {
		//Use swipe.js if mobile device & has touch capability & we're on the front page
			if($('#slideshow').length > 0 && window.device.mobileOS === true && window.device.touchOS === true) {
					var tablet_slider = new Swipe(document.getElementById('swipe'), {
						callback:function(event,index,elem) {
							//$('.tablet-tab').removeClass('current');
							//$('#tablet-tabs').find('li[data-index='+index+']').addClass('current');
						}
				});
			}
			//Else just use a normal slider
		  	...
		  	$('#slider').anythingSlider({
		  	        ...
		  	        onSlideBegin: function (e, slider) {
		  	            slider.$items.fadeOut(200);
		  	        },
		  	        onSlideComplete: function (slider) {
		  	            slider.$items.fadeIn(200, "swing");
		  	        }
		  	    });
		}
		
		function loginProcessed(response) {
		...
		$('.login-click').on('click', function(e) {
			...
			e.preventDefault();
			console.log('logging in via JS ');
			
			if(el.hasClass('browserid')) {
			  navigator.id.getVerifiedEmail(function(assertion) {
			      if (assertion) {
			        var params = {assertion: assertion};
			        OP.Util.makeRequest('/user/browserid/login.json', params, loginProcessed);
			      } else {
		...
		$('#edit-photo-form').on('submit',function(e) {
			e.preventDefault();
			...
			OP.Util.makeRequest('/photo/'+photo_id+'/update.json', data, editPhotoCallback);
			
		});
		
		function editPhotoCallback(response) {
			console.log(response);
			$('#photo-description').html(response.result.description);
			$('#photo-title').text(response.result.title);
