<ul class="nav">
	...
	<li id="nav-albums" <?php if($this->utility->isActiveTab('albums')) { ?> class="active" <?php } ?>>
...
<?php if($this->user->isLoggedIn()) { ?>
	<li id="nav-signin">
		...
		<div class="settings-click">
			...
			<ul id="settingsbar" class="wrapper">
...
<?php } ?>
	...
	<li id="nav-search" <?php if($this->utility->isActiveTab('search')) { ?> class="active" <?php } ?>>
		<form action="<?php $this->url->photosView(); ?>" method="get" id="form-tag-search">
			<input type="text" name="tags" placeholder="Search tags" class="select" /><button type="submit" class="search-click" title="Search now">Search</button>
		</form>
	</li>
	-->
</ul>
